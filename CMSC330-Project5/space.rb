  ...
  $simOut.each {|o|
    ...
    if o =~ /(\w+) (docking at|departing from) (\w+)/
      ...
      if (action == "board")
        ...
        person.set_itin_id(person.itinerary_idx + 1)
  ...
  $simOut.each {|o|
    if o =~ /(\w+) (docking at|departing from) (\w+)/
      ...
      if (action == "board")
        ...
        if port.ships.index(ship).nil?
          validSim = false
        elsif port != person.itinerary[person.itinerary.idx]
          validSim = false
        elsif (person.itinerary_idx > 1 && (person.itinerary[person.itinerary[person.itinerery_idx == person.itinerary[person.itinerary_idx - 1]))
          validSim = false
...
def travelerSimulator(trav)
  while ((trav.itinerary_idx != trav.itinerary.length - 1))
    ...
    destination_port.starMonitor.synchronize {
      ...
      destination_port.dockCondition.broadcast(0
    ...
    destination_port = trav.itinerary[trarv.itinerary_idx]
    sleep (0.001)
