...
def validate(filename)
  ...
  # Regular expression for the first hypen followed by the specified user
  # or another hyphen if no username is specified.
  hyp = '\s\-\s'
  uname = '(\-|(\w+))(\s)'
  
  # Regular expressions for the days, months, and years.
  days = '(3[0-1]|2[0-9]|1[0-9][0-9])'
  ...
  # A formatted expression concatenating day, month, year, the times, and the time
  # zone together.
  date = '\[' + days + '\/' + months + '\/' + years + '\:' + hours + '\:' + minutes + '\:' + seconds + '\s' + zone + '\]\s'

  # Regular expresions for the request arbitrary string, status code, and the
  # number of bytes sent through the request.
  requests = '\"(\\\"|[^"])+(\\\"|[^"\\\])\"\s'
  status = '(([0-9]+)\s)'
  bytes = '(\-|([0-9]+))'
  
  # The final concatenated regular expression that each line in the file is
  # compared to.
  ...
  # regular expression. If the line do not match or the line is not a newline
...
def time(filename)
  ...
  # Goes through each line and checks it against the newline character. The lines
  # will be split at the semicolons. We'll use our has to check hours against nil
  # and increment by 1 if they are not equal.
  lines.each{ |line|
    if line != "\n" then
      splits = line.split(" ")
      date = splits[3].split(":")
...
# This method will produce a list containing the top-ten most common requests
# strings reecieved by the web server.
def popularity(filename)
  ...
  while index <= 9
    if hasher.empty? == true then
      ...
      puts "#{maxiumum_value} #{maximum_key}\n"
...
def requests(filename)
  ...
  lines.each{ |line|
    # Use scan to get the entire line for backreferences
