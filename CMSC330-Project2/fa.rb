    ...
    @transition.keys.each{ |key|
      ...
      @transition[key].keys.each{ |value|
        if get_transition(key, value) != nil then
          ...
          transitions += get_transitions(key, value).size
          hasher[key] += v2
        end
      }
    }
      ...
      @alphabet.each{ |lettER|
        ...
        if !scap.empty?
          ...
          if state_change[r] == nil
            ...
            newDFA.add_transition(state_changer], state_change[epsilon], letter)
          end
        end
      }
    end
